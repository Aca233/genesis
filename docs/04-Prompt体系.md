...
结构合法后另过语义质量门：初审有 error 时，最多三轮调用批量局部修复规划器。规划器不接收也不得返回完整卡组，只接收问题路径的当前值、稳定引用目录和权威约束，并只能为本轮审计路径返回 `replace | remove` 操作；服务端硬性过滤未列出路径，每轮后重新执行完整本地校验，再只对已修改路径做语义复审。这样“多次调用”用于有上限地收敛残余问题，而不是重复上传或改写整份世界。
