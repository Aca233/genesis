...
export async function mutateAdminTask(ctx: AdminActionContext, input: { kind: "genesis" | "narrative" | "rewrite"; taskId: string; action: "cancel" | "retry" | "recover"; reason: string }, db: AdminDb = prisma) {
  ...
  if (input.kind === "genesis") {
    const task = await db.genesisTask.findUnique({ where: { id: input.taskId }, select: { id: true, userId: true, status: true, stage: true, leaseExpiresAt: true } });
    ...
    else { const result = await db.genesisTask.updateMany({ where: { id: task.id, ...(input.action === "retry" ? { status: "failed" } : { status: { in: ["running", "repairing"] }, leaseExpiresAt: { lt: now } }) }, data: { status: "queued", error: null, leaseToken: null, leaseExpiresAt: null, attempt: 0 } }); if (result.count !== 1) throw new Error("任务当前不可恢复"); ensureGenesisTaskRunning(task.id); }
