...
export function openingDirective(mode: WorldMode): string {
  if (mode === "creator") {
    return `(Director's note, not in-fiction input): This is the first passage of this world's recorded history. Open on a broad tableau of the world in its present era, then move into the most immediate world-internal conflict among its gods, peoples, factions, or characters. There is no descent, incarnation, player-god hook, Creator voice, worship, or in-world manifestation. End the prose on a concrete unfolding world-internal tension—an image, action or consequence—and offer observation/focus choices only in META suggestions. Never append an abstract fate/causality summary or a second-person call-to-action question. The final prose sentence must be declarative, never interrogative. Chinese prose; the current scale applies.`;
  }
  return `(Director's note, not in-fiction input): This is the first passage of this world's recorded history. Open with a genesis / descent set-piece: echo the player's primordial decree and the player god's starting situation, unveil the world at its present hour, and plant the opening hooks drawn from the player god's situation and the pantheon's visible tensions. End the prose on a concrete in-world image, action, response or consequence immediately before the player god's first act. Put possible actions only in META suggestions. Never append an abstract fate/causality summary or a second-person call-to-action question. The final prose sentence must be declarative, never interrogative. Do NOT act or speak for the player god. Chinese prose; the current scale applies.`;
}
