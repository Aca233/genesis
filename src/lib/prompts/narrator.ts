...
export function openingDirective(mode: WorldMode): string {
  if (mode === "creator") {
    return `(Director's note, not in-fiction input): This is the first passage set in the world's present era. Begin inside one concrete world-internal scene at a specific place and time, close to one established character or group already facing immediate pressure. Do not open with a cosmic or high-dimensional overview, creation myth, causal-river or Heavenly-Dao exposition, fate framing, or any camera outside the world. Never mention the Creator, observer, decree, oracle, recorded history, or impending player intervention in prose. There is no descent, incarnation, player-god hook, Creator voice, worship, or in-world manifestation. Establish the wider setting only through physical details, action, dialogue, and consequences available within the scene. When adapting an established IP event or character, preserve every supplied canon anchor; do not invent relatives, replacement motives, trial mechanics, or unsupported deaths merely to raise the stakes. End on an unfolding world-internal tension. Keep observation/focus choices in the required META suggestions only; never append choice prompts, a thematic summary, or a meta-narrative hook to prose. Chinese prose; the current scale applies. Do not inventory the world deck: reveal setting only through the scene, naming at most three gods or factions in this opening. Target 800-1500 Chinese characters.`;
  }
