...
3. CURRENT STATE ONLY (hard constraints): every card describes its subject AS OF the anchor moment, never a whole-life overview. Future identities, abilities, relations and knowledge must NOT be written as current state. Give every god, faction, race, place and major character the statusAtAnchor matching the anchor moment plus a one-sentence anchorNote for any non-default status. Ability timing: "at_anchor" for abilities held at the anchor, "future" for canon abilities acquired only later, "lost" for abilities already gone; an entity whose statusAtAnchor is active may only hold timing "at_anchor" abilities. AUTHORITY ORDER on any conflict: explicit player statements > player-uploaded lorebook > the locked temporal anchor > events of this playthrough > your own canon knowledge > your free speculation.
   ANCHOR SNAPSHOTS (slim, current-state only): where the anchor state is not obvious from the card's baseline text, fill the optional stateAtAnchor snapshot — majorCharacters[].stateAtAnchor {identity, locationRef (an existing place ref or null), factionMemberships [{factionRef, role, status active|former|secret}], currentGoals (at most 3), currentSituation, knowledgeHints (at most 3; hints, not an authoritative knowledge boundary)}; factions[].stateAtAnchor {leaderRefs, territoryRefs, currentStrength}; god stateAtAnchor {currentRank, currentDomains} when they differ from the card face. A snapshot records ONLY the state AS OF the anchor moment — never biography, never future turns.
...
8. If lorebook excerpts are provided, they are AUTHORITATIVE over your own knowledge on any conflict.
9. ALL user-facing string values must be written in Chinese. Keys stay English per schema.
   META-LANGUAGE BOUNDARY (hard constraint): The words 玩家/玩家神明 may appear ONLY in author-facing fields (agenda, stanceToPlayer, initialRelationToPlayer, narrationNotes, canonEvents). World-facing strings — ability effect/trigger/limitations/rumorText, faction/place/race overviews, catchphrases — must use in-world designations instead (the player god's name, 『体系外的意志』, 『不在名录中的存在』…), because these strings can surface verbatim in the codex and narration.
10. CANON FUTURE EVENTS (将临之事): fill canonEvents with 3-5 author-only candidate future events that loom beyond the opening moment. For an existing IP, use genuine not-yet-happened canon beats that lie after the current moment — canon beats beyond canonCutoff belong here and nowhere else; for an original world, derive looming candidates from epochConflict.overtConflicts and hiddenCurrents. Order the array by ordinal starting at 1, strictly increasing; the opening moment is ordinal 0, and every canonEvents ordinal must be strictly greater than anchorOrdinal. Every participantRef and condition ref must resolve to an existing card ref, and prior_event_occurred may only reference an earlier canonEvents entry. Give each event 1-3 prerequisites from the closed condition kinds, preferring concrete entity_status / relation_status / prior_event_occurred over custom. These are pressure, not a script: status is always "pending", visibility always "author_only", and no other card may state them as已定之事.`;
...
const TOP_LEVEL_ORDER: Record<WorldMode, string> = {
  pantheon: "mode, worldName, temporalAnchor, cosmology, fusionAxiom, playerGod, majorGods, minorGods, factions, races, places, majorCharacters, relationsAtAnchor, epochConflict, canonEvents, style, theme",
  creator: "mode, worldName, temporalAnchor, cosmology, fusionAxiom, majorGods, minorGods, factions, races, places, majorCharacters, relationsAtAnchor, epochConflict, canonEvents, style, theme",
};
