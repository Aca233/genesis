...
export async function buildGenesisMaterialSnapshot(selections: MaterialSelectionItem[]): Promise<GenesisMaterialSnapshot | null> {
  if (!selections.length) return null;
  const parsedSelections=selections.map(x=>MaterialSelectionItemSchema.parse(x));
  const versions=await prisma.materialVersion.findMany({where:{id:{in:parsedSelections.map(x=>x.materialVersionId)},card:{userId:"local"}},include:{card:true}});
  ...
  await prisma.materialCard.updateMany({where:{id:{in:items.map(x=>x.card.id)}},data:{lastUsedAt:new Date()}});
  return snapshot;
}
