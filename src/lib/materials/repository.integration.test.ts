...
describe("material repository", () => {
  it("archives initial cards idempotently and appends immutable copies", async () => {
    const world = await prisma.world.create({ data: { userId: "test-user", name: `materials-${crypto.randomUUID()}`, genesisInput: "test", lockedPaths: [] } });
    try {
      const deck = completeDeck();
      await prisma.$transaction((tx) => archiveInitialDeck(tx, "test-user", { worldId: world.id, worldName: world.name, deck }));
      await prisma.$transaction((tx) => archiveInitialDeck(tx, "test-user", { worldId: world.id, worldName: world.name, deck }));
      ...
      const copy = await createMaterialVersion("test-user", {
  ...
  it("Creator 归档不创建玩家神素材并保留全部主神能力", async () => {
    const world = await prisma.world.create({ data: { userId: "test-user", name: `creator-materials-${crypto.randomUUID()}`, genesisInput: "test", mode: "creator", lockedPaths: [] } });
    try {
