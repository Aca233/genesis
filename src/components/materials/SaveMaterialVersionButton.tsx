...
export function SaveMaterialVersionButton({ sourceType, sourceId, compact = false }: SaveMaterialVersionButtonProps) {
  ...
  return (
    <>
      ...
      {state.open && (
        <div className="fixed inset-0 z-[80] grid place-items-center bg-ink/35 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget && !state.pending) setState(closeSaveMaterialDialog); }}>
