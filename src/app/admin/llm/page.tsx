...
export default async function AdminLlmPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <AdminSection title="模型观测" note="只读调用元数据；不展示 Prompt、Response 或密钥">
    ...
    {data.items.length ? <div className="space-y-3">{data.items.map((call) => <article key={call.id} className="rounded-xl border border-gilt/20 bg-paper/45 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><strong>{call.provider ?? "未知提供商"} / {call.model ?? "未知模型"}</strong><span className={call.ok ? "text-emerald-700" : "text-cinnabar"}>{call.ok ? "成功" : "失败"}</span></div><p className="mt-1 text-xs text-ink-soft">{call.task} · {call.slot} · {call.durationMs}ms · {call.createdAt.toLocaleString("zh-CN")}</p><p className="mt-2 text-xs text-ink-soft">传输 {call.transportKind ?? "旧记录"} · 结果 {call.transportOutcome ?? "—"} · 终局证据 {call.terminalEvidence ?? "—"}{call.stableErrorCode ? ` · ${call.stableErrorCode}` : ""}</p><p className="mt-2 text-xs text-ink-soft">输入 {number(call.inputTokens)} · 输出 {number(call.outputTokens)} · 缓存读 {number(call.cacheReadTokens)} · 缓存写 {number(call.cacheWriteTokens)}</p>{call.error && <p className="mt-2 text-sm text-cinnabar">{call.error}</p>}<p className="mt-1 break-all text-[11px] text-ink-faint">逻辑调用 {call.logicalCallId ?? "—"} · 物理序号 {call.physicalAttemptIndex ?? "—"}</p><p className="mt-1 break-all text-[11px] text-ink-faint">用户 {call.userId ?? "—"} · 世界 {call.worldId ?? "—"}</p></article>)}</div> : <EmptyState>{hasFilters ? <>没有符合当前条件的记录。<Link href="/admin/llm" className="ml-2 underline">清除筛选</Link></> : "当前没有模型调用记录"}</EmptyState>}
