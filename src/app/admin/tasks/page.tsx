  ...
  return <AdminSection title="任务仪轨" note="任务输入与原始输出不会进入后台">
    <AdminFilter>
      ...
      <select name="status" defaultValue={status} className={inputClass}>
        <option value="all">全部状态</option><option value="queued">排队</option><option value="pending">执行中（叙事）</option><option value="running">执行中（创世）</option><option value="failed">失败</option><option value="cancelled">已取消</option><option value="completed">已完成</option>
      </select>
      <span />
    </AdminFilter>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:justify-between">
          <div>
            <div className="flex flex-wrap gap-2">
              ...
              {stale && <span className="rounded-full border border-cinnabar/35 px-2 py-0.5 text-xs text-cinnabar">疑似失租</span>}
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            />}
            {task.kind !== "narrative" && stale && <AdminActionButton
