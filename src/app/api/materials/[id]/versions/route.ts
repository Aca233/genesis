...
export const POST = withAuth(async (userId, request: Request, { params }: { params: Promise<{ id: string }> }) => {
  const { id } = await params; const parsed = Schema.safeParse(await request.json());
  if (!parsed.success) return NextResponse.json({ error: parsed.error.issues[0]?.message }, { status: 400 });
  try { return NextResponse.json({ version: await createMaterialVersion(userId, { cardId: id, ...parsed.data }) }, { status: 201 }); }
