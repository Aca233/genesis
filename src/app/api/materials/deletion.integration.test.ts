...
describe("material deletion lifecycle", () => {
  it("preserves cards, immutable versions and frozen task snapshots after source world deletion", async () => {
    const world = await prisma.world.create({ data: { userId: "test-user", name: `delete-source-${crypto.randomUUID()}`, genesisInput: "test", lockedPaths: [] } });
    let cardId = "";
    try {
      await prisma.$transaction((tx) => archiveInitialDeck(tx, "test-user", { worldId: world.id, worldName: world.name, deck: completeDeck() }));
      ...
      const task = await prisma.genesisTask.create({ data: { userId: "test-user", decree: "冻结素材测试", completedKeys: [], materialSelection: frozen } });
  ...
  it("rejects deleting the default or final version and never mutates old JSON", async () => {
    const world = await prisma.world.create({ data: { userId: "test-user", name: `delete-version-${crypto.randomUUID()}`, genesisInput: "test", lockedPaths: [] } });
    try {
      await prisma.$transaction((tx) => archiveInitialDeck(tx, "test-user", { worldId: world.id, worldName: world.name, deck: completeDeck() }));
      ...
      await expect(deleteMaterialVersion("test-user", initial.id)).rejects.toThrow("默认版本不可删除");
      const copy = await createMaterialVersion("test-user", { cardId: card.id, name: "复制版", parentVersionId: initial.id, content: initial.content as never, dependencies: initial.dependencies as never });
      await expect(deleteMaterialVersion("test-user", initial.id)).rejects.toThrow("默认版本不可删除");
      await expect(deleteMaterialVersion("test-user", copy.id)).resolves.toMatchObject({ id: copy.id });
