...
export const POST = withAuth(async (userId, _request: Request, { params }: { params: Promise<{ id: string }> }) => {
  const { id } = await params;
  const world = await prisma.world.findFirst({ where: { id, userId }, select: { materialArchiveStatus: true } });
