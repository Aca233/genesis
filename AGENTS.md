...
`mcp__fastctx__glob` — prefer them over shell-based alternatives. Pass
absolute paths and continue with the exact parameters reported by a
`Partial` result.

### Batch replacement

Use `mcp__fastctx__replace` only for deterministic mechanical replacements.
Always run it with `dry_run: true` first and inspect the preview before
writing.
<!-- fastctx:end -->

## Global editing policy

- Generated code, semantic rewrites, and small local edits MUST use the session-injected `apply_patch` helper.
- Invoke the helper directly through `exec_command`; do not wrap it in Python, Node.js, generated PowerShell/Bash scripts, redirection, heredocs, or temporary patch files.
- Shell commands may be used for tests, builds, type checks, linters, Git inspection, and operations that cannot be performed with a dedicated tool; they must not be used to rewrite source files.
- If neither the session-injected `apply_patch` helper nor FastCtx is available, stop before modifying files and report that the required editing tools are unavailable.
- Do not delegate code modification to a subagent unless the user explicitly requests delegation. Any authorized subagent must receive and follow this same editing policy.
